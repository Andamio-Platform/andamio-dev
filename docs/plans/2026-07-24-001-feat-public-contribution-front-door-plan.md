...
**Boundary & ownership**
- R8. andamio-dev never hosts, mirrors, or exposes the private coordination (triage, priority, delegation, sequencing rationale).
- R9. andamio-dev assumes no code-quality/security accountability for credential-badges (that ownership stays Development's).
...
- **Two boundary invariants, enforced differently.** The *no-private-coordination-leak* invariant (R8) is structural: the live view reads only a fixed set of *public* issue fields via `gh` (title, number, url, state, labels, body), and the private coordination is never queried, so no code path can source private signal. The *no-andamio-dev-prioritization* invariant (R5) is weaker — because `/contribute` is an LLM-driven skill, nothing structurally prevents the agent from ranking or editorializing when asked "which first?". The skill therefore carries an explicit instruction to present fetched public fields as-is and never characterize priority/urgency/difficulty in its own words (U2).
...
flowchart TB
  ...
  BOARD["private coordination side<br/>(PRIVATE)"]
...
- **Test scenarios:**
  ...
  - Boundary: no field in any entry encodes priority, triage, delegation, or the private coordination status (grep the schema/keys) — Covers AE1 at the data layer.
...
- **Test scenarios:**
  ...
  - Boundary (Covers AE1): given an open issue carrying labels set via private coordination delegation, the skill shows the issue and its already-public labels but adds no priority/triage annotation of its own.
  - Boundary (Covers R8): the skill never references or queries the private coordination/the private coordination workspace; a read of the skill body confirms no private source is consulted.
...
- **Test scenarios:**
  ...
  - Boundary (Covers R8): the create path applies no privately-derived labels or priority fields.
...
- **Test scenarios:**
  ...
  - Boundary (Covers R3): a read confirms no private knowledge (the private coordination, triage, priority, product wants) appears; content is safe for any public reader.
...
- **Privacy boundary (cross-cutting).** This is the load-bearing concern: andamio-dev becomes a surface that *reads from and writes to* a separate public repo. The invariant — only public GitHub data in, no the private coordination anything, no andamio-dev-added prioritization — is enforced structurally (U1 schema carries only public fields; U2/U3 query only `gh`). Any future repo added to the registry inherits the same boundary for free.
...
- **Prerequisite, not a blocker to building (from origin).** Promoting/announcing the front door follows the private coordination side being proven in the private coordination workspace (the private-side coordination work + triage operation). The artifact here uses only public info and can be built and merged now; "going live" as the promoted front door is a separate activation step.
...
- Owning code-quality/security for public leaf repos — code-quality/security ownership stays Development's.
...
- Whether credential-badges currently defines issue templates the create path should adopt, and the minimum fields to prompt for (check the live repo at build time; must not encode the private coordination semantics).
...
- Handoff context: `andamio/docs/handoff-andamio-dev-public-face.md` — the layered surface-work system, the code-quality/security-ownership rule, credential-badges-first.
