...
- **Live "what's open" from public GitHub only.** The available-work view reflects credential-badges' open GitHub issues, read via public GitHub. Board-29 is never a source, and andamio-dev adds no prioritization signal of its own — it shows only what is already public on the issue.
- **credential-badges' issues are the work queue; anybody executes.** Work is logged as public GitHub issues and picked up by anybody — James via compound-engineering directly in the repo, or a new contributor. Board-29 triages and prioritizes privately, *after* an issue exists.
- **Issue filing under the contributor's own identity.** New issues are created under the contributor's own GitHub account (their `gh` auth), not a shared andamio-dev bot — clean attribution, and it keeps P-022 unambiguous.
- **P-022 not transferred.** Being the public face does not make andamio-dev accountable for credential-badges' code quality or security; that stays Development's.
...
- A4. Development — owns code-quality/security accountability (P-022) on public repos; coordinates private board-29 triage in product-circle.
- A5. Board 29 / product-circle (private) — where triage, priority, and delegation happen; never surfaced by A2.
...
- F3. File a new issue
  ...
  - **Actors:** A1, A2, A3
  - **Steps:** Skill helps shape the idea into an issue → files it into credential-badges under the contributor's own GitHub identity → returns the issue link. Board-29 triage happens privately afterward, outside andamio-dev.
  - **Covered by:** R6, R7, R8
...
- R8. andamio-dev never hosts, mirrors, or exposes board-29 coordination (triage, priority, delegation, or sequencing rationale) — those stay in product-circle.
- R9. andamio-dev assumes no code-quality or security accountability for credential-badges (P-022 stays Development's); its role is limited to orientation, live tracking, and issue logging.
...
- AE1. **Covers R5, R8.** Given a credential-badges issue that carries labels or status set privately through board-29 delegation, When `/contribute` shows the live view, Then it shows the issue and whatever is already public on it and adds no priority/triage annotation of its own.
...
- Owning code-quality or security for public leaf repos — P-022 stays Development's.
...
- **Private side is a prerequisite to activation, not to building.** Promoting/announcing the front door follows the private board-29 coordination in product-circle being proven (PR #124 + the triage operation). The public artifact itself can be built now — it uses only public information.
...
- What issue template or fields (if any) are applied on filing, and how to prompt for credential-badges' minimum expected fields — without encoding any board-29 semantics.
...
- `andamio/docs/handoff-andamio-dev-public-face.md` — the layered surface-work system, the one rule (no private coordination in a public repo), P-022, and credential-badges-first scope.
...
- Private context (not to be surfaced publicly): `product-circle/.claude/skills/request-dev-capacity/` (board-29 references and triage operation) and `andamio/docs/reconcile-board-29-in-product-circle.md`.
