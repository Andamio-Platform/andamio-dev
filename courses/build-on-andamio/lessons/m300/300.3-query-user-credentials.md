...
module: 300
order: 3
title: "Query a user's Andamio credentials"
...
The most common read in Andamio integrations: fetching a user's credentials and interpreting what the response says. By the end you can answer "which courses has this user completed?" and "which modules has this user earned credit for?" for the alias you are authenticated as.

## The endpoint

`POST /v2/course/student/credentials/list`

POST for a read — same pattern from M300.2. There is no request body: the route returns the credential record of the alias whose User JWT you send. Auth: API key and User JWT, both required (M200.2).

## The call

The Andamio CLI is a tool for owners, teachers and managers, so a learner's own reads go straight to the API. Export your API key and User JWT as `ANDAMIO_API_KEY` and `ANDAMIO_JWT`, then:
...
If you haven't enrolled in any courses yet, the `data` array will be empty — that's fine, the response still shows the shape.
...
curl -s -X POST https://preprod.api.andamio.io/api/v2/course/student/credentials/list \
  -H "X-API-Key: $ANDAMIO_API_KEY" \
  -H "Authorization: Bearer $ANDAMIO_JWT" \
  | jq -r '.data[] | . as $course
           | $course.modules[]
...
**Lesson 300.4: Map each tx_type to its build endpoint.** The full catalog of write operations and how to find the build endpoint for any of them.
