...
{
  "alias": "$MY_ALIAS",
  "initiator_data": "$MY_ADDRESS"
}
EOF
```

`initiator_data` is an exception to Andamio's normal request shape. Every other write endpoint looks up your address from your alias. The mint is different — no alias exists yet, so you provide your bech32 address explicitly as a plain string. (Path A sponsorship uses `sponsor_data` instead — provide one or the other, not both.)
...
**Lesson 200.2: API keys and User JWTs.** You have an alias. Next: the two credentials that authenticate it against the API.
