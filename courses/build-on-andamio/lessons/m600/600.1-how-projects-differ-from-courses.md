...
module: 600
order: 1
title: "How Andamio projects differ from courses"
...
| **Deposit** | ~2.3 ADA (enrollment) | ~2.3 ADA (contributor state) |
| **Assessment outcomes** | Accept, Refuse | Accept, Refuse, **Deny** |
| **Credential claim** | Net positive (~1.65 ADA back) | Net positive (deposit refund + task reward − commission, 5% with a 1 ADA minimum) |
...
This is why projects require more ADA up front than courses. Creating a project costs ~148 ADA in service fee, deposits and tx fee, plus whatever you deposit into the treasury. A course costs ~121 ADA to set up and has no treasury.
...
The escrow is what makes projects trustless. The contributor knows the reward exists before committing. The manager knows the contributor has skin in the game (~2.3 ADA locked at commit). Neither party has to trust the other — the protocol enforces the deal.
...
**Lesson 600.2: Compute a `task_hash` for a project task.** Courses have `slt_hash`. Projects have `task_hash`. Same idea, different encoding — next lesson explains both the how and the why.
