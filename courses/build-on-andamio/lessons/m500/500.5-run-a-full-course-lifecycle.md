...
Creates the course on-chain: mints the LocalStateNFT (whose policy ID becomes the `course_id`), LocalStateToken, and governance token. Cost: ~20.7 ADA (1 ADA service fee + ~19 ADA in minUTxO deposits + ~0.5 ADA tx fee). Under Pricing v3 (shipped 2026-04-09) the service fee is flat — it no longer scales with teacher count.
