...
module: 500
order: 5
title: "Run a full course lifecycle on Andamio"
...
You know the structure, the hash, the publishing flow, and the commitment lifecycle. This lesson puts them together: create a course, publish a module, enroll a student, submit evidence, assess it, and claim a credential. Six on-chain transactions, two loops.
...
To run the steps by hand, set these first:
...
```bash
COURSE_ID=$(andamio tx run /v2/tx/instance/owner/course/create \
  --body '{"alias":"'"$ALIAS"'","teachers":["'"$ALIAS"'"]}' \
  --skey "$SKEY_PATH" \
  --tx-type course_create \
  --output json | jq -r '.build_response.course_id')
```

Creates the course on-chain: mints the LocalStateNFT (whose policy ID becomes the `course_id`), LocalStateToken, and governance token. Cost: ~119.3 ADA (100 ADA service fee + ~18.8 ADA in minUTxO deposits + ~0.56 ADA tx fee). Name every teacher here: a teacher added after creation pays a further 10 ADA service fee.

The builder returns the new `course_id` in its response, so the command above captures it once the transaction reaches `updated`.
...
```bash
# On-chain: enrollment + initial evidence (this also creates the commitment)
...
Cost: ~2.64 ADA (includes ~2.3 ADA locked in the course-state UTxO, mostly returned at credential claim). When it reaches `updated`, the commitment exists and is already `SUBMITTED` — there is no separate create step. The `assignment_info` field is the student's initial evidence — it can be updated before the teacher assesses.
...
Cost: ~0.27 ADA, the network fee. The `assignment_decisions` array can include multiple students in one transaction (batch assessment from M500.4).
...
Cost: **net negative**. The student gains ~1.65 ADA because the enrollment deposit is refunded and the CourseState token is burned. A credential claim is the only transaction where the initiator ends with more ADA than they started.

The credential NFT is now in the student's wallet — permanent, portable, tied to the `slt_hash`.

## Cost summary

The `course.credential` loop costs ~1.27 ADA total (student: ~0.99 ADA net; teacher: ~0.27 ADA). For a cohort of N students, the cost scales linearly: ~1.27 ADA × N.

The `course.setup` loop costs ~121 ADA for a one-module course. Setup is a one-time cost; the credential loop runs per student per module.

| Transaction | Paid by | Cost |
|-------------|---------|------|
| `course_create` | Owner | ~119.3 ADA |
| `modules_manage` | Teacher | ~1.86 ADA |
| `assignment_submit` | Student | ~2.64 ADA |
| `assignment_update` | Student | ~0.33 ADA (optional, per update) |
| `assessment_assess` | Teacher | ~0.27 ADA |
| `credential_claim` | Student | -1.65 ADA (refund) |
...
If you're running on a wallet with limited test ADA, the setup loop alone costs ~121 ADA. Hit the faucet for at least 150 test ADA before starting.
...
Before Module 600, there's the Module 500 assignment. If you ran the lifecycle and answered each "Your turn," most of the evidence is already in hand.

Open `lessons/m500/assignment.md` or go to the assignment page in the Andamio app.
