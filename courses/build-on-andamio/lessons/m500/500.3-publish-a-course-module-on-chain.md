...
```bash
andamio course create-module --course-id "$COURSE_ID" \
  --code 100 \
...
| Field | Purpose |
|-------|---------|
| `modules_to_add` | Array of modules to mint. Each entry has `slts`, `allowed_student_state_ids`, and `prereq_credential_hashes`. |
...
| `prereq_credential_hashes` | Credential prerequisites. Empty = no prerequisites. |
