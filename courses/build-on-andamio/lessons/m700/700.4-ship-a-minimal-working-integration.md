...
module: 700
order: 4
title: "Ship a minimal working Andamio integration"
lesson_type: developer_documentation
bloom_level: apply
estimated_time_min: 10
prerequisites: ["700.3"]
unlocks: ["700.5"]
evidence_type: repo_link
---
...
# 2. Query credentials
curl -X POST https://preprod.api.andamio.io/api/v2/course/student/credentials/list \
...
# Summarize where each student stands in a course you teach
andamio teacher assignments list --course "$COURSE_ID" --output json \
  | python3 -c "
import json, sys
from collections import Counter
rows = json.load(sys.stdin)['data']
for status, n in Counter(r['content']['commitment_status'] for r in rows).items():
    print(f'{status}: {n}')
"
```
...
**Lesson 700.5: Critique the Andamio developer experience.** Last lesson. You've built on Andamio end to end. Now: what would you change?
