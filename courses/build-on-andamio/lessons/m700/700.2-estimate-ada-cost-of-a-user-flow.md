...
**Formula:** ~119.3 + (1.32 × N)

| Students | Setup | Credentials | Total | Per-student |
|----------|-------|-------------|-------|-------------|
| 1 | 119.3 | 1.32 | ~121 | ~121 |
| 10 | 119.3 | 13.20 | ~133 | ~13.3 |
| 30 | 119.3 | 39.60 | ~159 | ~5.3 |
| 100 | 119.3 | 132.00 | ~251 | ~2.5 |

The per-student cost drops as the cohort grows because setup is amortized. At 100 students, setup is still ~48% of the total. At 1000 students, it falls to ~8%.
...
**Formula (non-recoverable costs):** ~101.3 + (0.43 × M) + (1.21 × N) + commission
...
Total:                          ~410 ADA
Per student (all 7 modules):    ~410 / 30 = ~13.7 ADA
Per credential:                  ~1.32 ADA
```

On preprod this is free test ADA. On mainnet, ~410 ADA at current prices is a real budget item — but it covers 210 on-chain credentials for 30 students.
...
**Total out of wallet:** ~729 ADA. **Recoverable:** treasury remainder (~0 if all tasks claimed), contributor deposits (~72.5), and some minUTxO deposits. **Non-recoverable:** 100 ADA service fee, ~8.5 ADA tx fees, 5% commission on rewards (5 × 5 ADA = 25 ADA). Non-recoverable total: ~134 ADA.
